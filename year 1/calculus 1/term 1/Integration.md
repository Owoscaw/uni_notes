...
When the region $D$ is rotationally symmetric, or has some kind of circular symmetry to it, polar coordinates could be useful to describe the region where $x=r\,cos\theta$ and $y=r\,sin\theta$. To convert a region in terms of cartesian coordinates into a region in terms of polar coordinates, the area element $dA=dxdy$ needs to be converted into polar coordinates. $dA$ describes the area of a rectangle with lengths $dx$ and $dy$, the area element in the infinitesimal limit of the integral. Consider a point $(r,\theta)$, extending $r$ by $dr$ and $\theta$ by $d\theta$:![[polar simple example]]
The approximation $dA\approx rd\theta\times dr$ gets better and better as $d\theta$ and $dr$ get smaller and smaller. In the infinitesimal limit of the integral, this becomes exactly $dA=rdrd\theta$. To compute the double integral using polar coordinates, $dxdy$ is replaced with the new area element, $rdrd\theta$. If the region is either $r$-simple or $\theta$-simple, the double integral is computed as an iterated integral ($r$ simple is red, $\theta$ simple is blue):![[polar simple lines example]]
Example:
![[polar double integral example]]
...
Giving us the result $dA=rdrd\theta$, used in [[year 1/calculus 1/term 1/Integration#Integration using polar coordinates|polar coordinate double integrals]]. Here is another example for using the Jacobian: ![[Jacobian example]]
