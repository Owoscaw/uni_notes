...
Similarly, consider:
$$ \frac{1}{L}\int_{-L}^L\sin{\frac{m \pi x}{L}}f(x)dx=\frac{1}{L}\int_{-L}^L\left(\frac{a_0}{2}+\sum_{n=0}^\infty a_n\cos{\frac{n \pi x}{L}}+b_n\sin{\frac{n \pi x}{L}}\right)\sin{\frac{m \pi x}{L}}dx=\sum_{n=1}^\infty b_n\delta_{mn}=b_m$$
Therefore we get the other coefficient with:$$\Huge b_n=\frac{1}{L}\int_{-L}^L\sin{\frac{n \pi x}{L}}f(x)dx$$
