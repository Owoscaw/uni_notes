...
The approximation $dA\approx rd\theta\times dr$ gets better and better as $d\theta$ and $dr$ get smaller and smaller. In the infinitesimal limit of the integral, this becomes exactly $dA=rdrd\theta$. To compute the double integral using polar coordinates, $dxdy$ is replaced with the new area element, $rdrd\theta$. If the region is either $r$-simple or $\theta$-simple, the double integral is computed as an iterated integral:![[Integration .excalidraw]]
