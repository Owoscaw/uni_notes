...
\epsilon\sin \frac{x}{\epsilon}&\epsilon>0\end{cases}$$Note that the notation $u^\epsilon$ is solely to emphasise dependence on $\epsilon$. We can then interpret $u^\epsilon(x,0)=0,\partial_tu^\epsilon(x,0)=g^\epsilon(x)$ as initial conditions. There exists a unique solution for this equation for any $\epsilon\geq0$, however notice that:$$\Huge ||g^\epsilon-g^0||_{L^\infty(\Re)}=\sup_{x\in\Re}|g^\epsilon(x)-g^0(x)|=\sup_{x\in\Re}|\epsilon\sin \frac{x}{\epsilon}|=\epsilon$$So consequently:$$\Huge ||g^\epsilon-g^0||_{L^\infty(\Re)}\to_{\epsilon\to 0}0$$That is, the initial data converges to $g^0$ uniformly in $\epsilon$. We then look at the other case, where we ask if the solution for $\epsilon>0$ converges uniformly to $u^0$ when $\epsilon\to0$. Notice that $g^0=0$ is a solution that satisfies all initial conditions, so the solution with given boundary conditions is $u^0=0$. To find $u^\epsilon$ for $\epsilon>0$ we use separation of variables and seek a solution of form:$$\Huge u^\epsilon(x,t)=X(x)T(t)\implies u^\epsilon(x,t)=\epsilon^2\sin\left(\frac{x}{\epsilon}\right)\sinh\left(\frac{t}{\epsilon}\right)$$We claim that this doesn't converge uniformly to $u^0=0$:$$\large\sup_{x\in\Re,t>0}|u^\epsilon(x,t)-u^0(x,t)|=\epsilon^2\sup_{x\in\Re,t>0}|\sin\frac{x}{\epsilon}\sinh\frac{t}{\epsilon}|=\epsilon^2\sup_{t>0}|\sinh\left(\frac{t}{\epsilon}\right)|=\infty$$as $|\sinh x|\to_{x\to\infty}\infty$. Fixing $t>0$ we see:$$ \sup_{x\in\Re,t>0}|u^\epsilon(x,t)-u^0(x,t)|=\epsilon^2\sup_{x\in\Re,t>0}|\sin\frac{x}{\epsilon}\sinh\frac{t}{\epsilon}|=\epsilon^2|\sinh \frac{t}{\epsilon}|=\frac{\epsilon^2(e^\frac{t}{\epsilon})-e^{-\frac{t}{\epsilon}}}{2}\to_{\epsilon\to0}\infty$$We ask if this even converges pointwise, so define for fixed $x\neq0$ and $t>0$:$$\Huge \epsilon_n=\frac{|x|}{2\pi n+\pi/2}$$Then we see:$$\Huge\begin{align*}
|u^{\epsilon_n}(x,t)-u^0(x,t)|&=\epsilon_n^2|\sin \frac{x}{\epsilon_n}\sinh \frac{t}{\epsilon_n}|\\
&=\epsilon_n^2|\sin\left(2\pi n+\frac{\pi}{2}\right)||\sinh \frac{t}{\epsilon_n}|\\
&=\epsilon^2_n|\sinh \frac{t}{\epsilon_n}|_{n\to\infty}\to \infty
\end{align*}$$