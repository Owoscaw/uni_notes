...
We have so far assumed that $T>0$ is a constant and that $\Omega\subset\Re^n$ is open and bounded. We define$$\Huge\Omega_T=\Omega\times(0,T]$$and we refer to $\Omega_T$ as a cylinder. The parabolic boundary of $\Omega_T$ is defined by$$\Huge \Gamma_T=\bar\Omega_T\setminus\Omega_T=(\Omega\times\{0\})\cup(\partial\Omega\times[0,T])$$which is the bottom and the sides of the cylinder, but not the interior or the top.

We define $C_1^2(\Omega_T)$ as the space of functions on $\Omega_T$ that are once continuously differentiable in time and twice continuously differentiable in space:$$\Huge C_1^2(\Omega_T)=\{u:\Omega_T\rightarrow\Re:u,u_t,u_{x_i},u_{x_ix_j}\in C(\Omega_T)\}$$Recall that a matrix $A\in\Re^{m\times m}$ is negative semi-definite if$$\Huge \underline{y}\cdot A\underline{y}\leq0\,\,\forall \underline{y}\in\Re^m$$, also recall that if $y_0\in\Re$ is a local maximum point of $g\in C^2(\Re)$, then $g'(y_0)=0$ and $g''(y_0)\leq0$. The $n$-dimensional generalisation of this result is the following. Let $U\subseteq\Re^m$ be open and let $g\in C^2(U)$. Suppose that $\underline{y}_0\in U$ is a local maximum point of $g$. Then$$\Huge\underline{\nabla}g(\underline{y}_0)=0,\,\,D^2g(\underline{y}_0)\text{ is negative and semi-definite}$$where $D^2g$ is the matrix of second partial derivatives of $g$. 

We can now state the weak maximum principle for the heat equation. Let $k>0$ and $\Omega\subset\Re^n$ be open and bounded. Let $u:\bar\Omega_T\rightarrow\Re,u\in C_1^2(\Omega_T)\cap C(\bar\Omega_T)$:
> If$$\Huge u_t-k\Delta u\leq0\text{ in }\Omega_T$$then:$$\Huge\max_{\bar\Omega_T}u=\max_{\Gamma_T}u$$That is, if $u$ is a solution of the heat equation with the source term $f\leq0$, then it attains its maximum on the parabolic boundary $\Gamma_T$.
> If$$\Huge u_t-k\Delta u\geq0\text{ in }\Omega_T$$then:$$\Huge\min_{\bar\Omega_T}u=\min_{\Gamma_T}u$$That is, if $u$ is a solution of the heat equation with the source term $f\geq0$, then it attains its minimum on the parabolic boundary $\Gamma_T$.
