...
We now show that the PDE holds. To do this, we need to differentiate $u$ wrt $\underline{x}$ and as such $\tau,\underline{s}$ wrt to $\underline{x}$. By the inverse function theorem:$$\Huge D(\tau,\underline{s})|_\underline{X}\cdot D\underline{X}|_{(\tau(\underline{x}),\underline{s}(\underline{x}))}=\mathbb{I}$$which can be shown trivially. Next:$$\Huge \underline{a}(\underline{x},u(\underline{x}))\cdot\underline{\nabla}u(\underline{x})=\sum_{l=1}^na_l(\underline{x},u(\underline{x}))\cdot$$