...
We have discussed Lorentz scalar fields, for which only the argument is transformed. Consider that the field itself is transformed. A field $A^\mu$ is a Lorentz vector if its behaviour under Lorentz transformations is:$$\Huge $$