...
\end{align*}$$for some constant $\lambda\in\Re$. The solutions therefore depend on $\lambda$:
> For $\lambda>0$ we can write $\lambda=\alpha^2>0$, so for the $\theta$-mode:$$\Huge-\frac{\Theta''}{\Theta}=\alpha^2\implies\Theta''+\alpha^2\Theta=0$$which leads to simple harmonic motion:$$\Huge \Theta(\theta)=C\cos(\alpha\theta)+D\sin(\alpha\theta)$$Note that velocity should be $2\pi$ periodic, therefore $\Theta$ must be $2\pi$ periodic and $\alpha=n$. This makes the $r$-mode:$$\Huge \frac{r}{R}(rR')'=n^2\implies r^2R''+rR'-n^2R=0$$which is a Cauchy-Euler type second order ODE. Substituting in $R=ar^m$ gives:$$\Huge (m(m-1)+m-n^2)ar^m=0\implies m^2=n^2\implies m=\pm n$$which makes the solution:$$\Huge R(r)=Ar^n+Br^{-n}$$
> For $\lambda=0$, the $\Theta$ mode simplifies to:$$\Huge \Theta''=0\implies\Theta(\theta)=C_0\theta+D_0$$and the $R$ mode simplifies to:$$\Huge rR'=A_0\implies R'=\frac{A_0}{r}\implies R(r)=A_0\log(r)+B_0$$However we require that $\underline{u}$ is periodic, that is we want:$$\Huge \underline{u}=\underline{\nabla}\phi=\Theta R'\hat{\underline{e}}_r+\frac{1}{r}R\Theta'\hat{\underline{e}}_\theta$$to be periodic. The second term is already periodic as $\Theta'$ is constant. For the first term, we can allow for a non-periodic $\Theta$ by setting $R'=0$. This means that either $A_0$ or $C_0$ must be $0$.
> For $\lambda<0$ the $\Theta'$ periodic condition fails.

Therefore the general solution to Laplace's equation becomes:$$\large \phi(r,\theta)=(A_0\log(r)+B_0)(C_0\theta+D_0)+\sum_{n=1}^\infty(A_nr^n+B_n^{-n})(C_n\cos(n\theta)+D_n\sin(n\theta))$$
## Common potential flows:
The only axis-symmetric potential, $\phi=\phi(r)$ is the solution to the above equation when $n=0$:$$\Huge \phi(r)=m\log(r)+C$$for constants $m,C$. The integration constant $C$ is arbitrary so we set it to $0$, making the velocity:$$\Huge\underline{u}=\underline{\nabla}\phi=\frac{\partial \phi}{\partial r}\hat{\underline{e}}_r+\frac{1}{r}\frac{\partial \phi}{\partial \theta}\hat{\underline{e}}_\theta=\frac{m}{r}\hat{\underline{e}}_r$$We call this the line source ($m>0$) or sink $(m<0)$ with strength $m$:![[Kinematics of Fluids 2025-10-29 14.43.12.excalidraw]]The only solution to Laplace's equation with only $\theta$ dependency is again when $n=0$:$$\Huge \phi(\theta)=k\theta+C$$where we set $C=0$. The velocity then becomes:$$\Huge \underline{u}=\underline{\nabla}\phi=\frac{\partial \phi}{\partial r}\hat{\underline{e}}_r+\frac{1}{r}\frac{\partial \phi}{\partial \theta}\hat{\underline{e}}_\theta=\frac{k}{r}\hat{\underline{e}}_\theta$$which is exactly the line vortex, centered at the origin.

A uniform flow also satisfies Laplace's equation:$$\Huge \phi(x)=Ux\implies\underline{u}=U\hat{\underline{e}}_x$$which clearly solves Laplace's equation.

## Flow past a cylinder:
Consider a static cylinder of radius $a$ in a uniform flow with velocity $(U,0)$ at infinity. No flow can pass through the cylinder so we ask what the velocity looks like everywhere else in the flow. The upstream condition implies that away from the cylinder, we expect $\underline{u}=(U,0)$, or $\phi=Ux$, we write this in polar coordinates as:$$\Huge \phi(r\to\infty)=Ux=Ur\cos\theta$$The no-normal-flow condition applies on the surface of the cylinder, and the normal to the cylinder is in the radial direction:$$\Huge \underline{\nabla}\phi\cdot\underline{\hat{n}}\vert_{r=a}=\frac{\partial \phi}{\partial r}\vert_{r=a}=0$$
We now look for general solutions to Laplace's equation that fit these conditions. The only part that fits the $r\to\infty$ condition is:$$\Huge \phi(r,\theta)=(A_1r+B_1r^{-1})(C_1\cos\theta)$$And comparing this to our constants gives:$$\Huge \phi(r,\theta)=(Ur-B_1r^{-1})\cos\theta$$Now for the condition on the cylinder's surface we have that at $r=a$:$$\Huge \frac{\partial \phi}{\partial r}=(U-B_1a^{-2})\cos\theta=0\implies B_1=Ua^2$$making the final so u lution:$$\Huge\phi=U\left(1+\frac{a^2}{r^2}\right)r\cos\theta$$we can also plot $\underline{u}$:![[Kinematics of Fluids 2025-10-29 15.03.13.excalidraw]]The points in orange are called "stagnation points". Note that this also defines a valid flow inside of the cylinder, however we ignore this for our context as this only exists as a virtual flow to meet the boundary condition along the cylinder.

## Flow past a rotating cylinder:
Letting the cylinder rotate, recall that we can add solutions to Laplace's equation. To construct a rotating cylinder flow, we can simply add the above flow to the line vortex:$$\Huge \phi=U\left(1+\frac{a^2}{r^2}\right)r\cos\theta+\frac{\Gamma}{2\pi}\theta$$Since we added a function of only $\theta$, the no-normal-flow condition still holds around the boundary of the cylinder. To understand the flow, we can look for stagnation points. To do this, we first find the velocity associated with this potential:$$\Huge\begin{align*}
\underline{u}=\underline{\nabla}\phi&=\frac{\partial \phi}{\partial r}\hat{\underline{e}}_r+\frac{1}{r}\frac{\partial \phi}{\partial \theta}\hat{\underline{e}}_\theta\\
&=U\left(1-\frac{a^2}{r^2}\right)\cos\theta\hat{\underline{e}}_r+\left(\frac{\Gamma}{2\pi r}-U\left(1+\frac{a^2}{r^2}\right)\sin\theta\right)\hat{\underline{e}}_\theta
\end{align*}$$Now we look for where $u_r=u_\theta=0$. Clearly $u_r=0$ at $r=a$, then $u_\theta=0$ if:$$\Huge\Gamma=4\pi Ua\sin\theta$$So our stagnation points are at:$$\Huge r=a,\,\,\sin\theta=\frac{\Gamma}{4\pi Ua}$$Choosing $\Gamma<0$ forces the stagnation points to be at $\theta<0$:![[Kinematics of Fluids 2025-10-29 15.17.06.excalidraw]]We see that most of the flow is carried over the top of the cylinder. Note $|\sin\theta|\leq1$ forces the condition on $\Gamma$ to be $|\Gamma|<4\pi Ua$, assuming $U>0$.

The other option for $u_r=0$ is when $\cos\theta=0$ at $\theta=\pm\pi/2$. Under this condition setting $u_\theta=0$ gives the condition:$$\Huge r^2\mp\frac{\Gamma}{2\pi U}r+a^2=0$$Since $\Gamma<0$ we have all positive coefficients and so there are no positive real solutions for $r$, so we discard this case and look at $\theta=-\pi/2$. This gives the positive branch of this equation, leading to solutions:$$\Huge r=a\left(\frac{|\Gamma|}{4\pi Ua}\pm\sqrt{\left(\frac{\Gamma}{4\pi Ua}\right)^2-1}\right)$$Given $|\Gamma|>4\pi Ua$ we get two real positive roots, one greater than $a$ and one less than $a$. Obviously we discard the solution with $r<a$ as this represents virtual flow within the cylinder. Taking the $r>a$ solution causes the flow to look like:![[Kinematics of Fluids 2025-10-30 02.25.42.excalidraw]]Where we see that the stagnation point is now below the cylinder. Here, the cylinder will move leftwards with backspin. This is known as the Magnus effect.
## Method of images:
Adding virtual sources/sinks/vortices into $\Re^2$ to replicate the effect of a more complex fluid domain can make solving Laplace's equation in complex fluid domains much easier. Such methodology is known as the method of images. The classic example is of a line source near a wall.

Suppose we have a line source of strength $m$ placed at $(x,y)=(d,0)$ near a wall at $x=0$:![[Kinematics of Fluids 2025-10-30 02.32.50.excalidraw]]
Disregarding the wall, the potential from the source is:$$\Huge\phi=m\log(|\underline{x}-(d,0|)=\frac{m}{2}\log((x-d)^2+y^2)$$This makes the $x$-component of the flow:$$\Huge u=\frac{\partial \phi}{\partial x}=\frac{m}{2}\frac{2(x-d)}{(x-d)^2+y^2}$$which clearly has $u\neq0$ at $x=0$, so the boundary condition at the wall $\underline{u}\cdot\underline{\hat{n}}=0$ is clearly not satisfied. To remedy this, we place a source of equal strength $m$ outside of the domain, reflected by the wall at $(-d,0)$. By symmetry, this source will create an opposite velocity and should cancel out the real sources velocity in the $x$-direction. The combined velocity potential is:$$\Huge\phi=\frac{m}{2}(\log((x-d)^2+y^2)+\log((x+d)^2+y^2))$$with:$$\Huge u=\frac{\partial \phi}{\partial x}=\frac{m}{2}\left(\frac{2(x-d)}{\left(x-d\right)^2+y^2}+\frac{2(x+d)}{(x+d)^2+y^2}\right)$$which, setting $x=0$, gives:$$\Huge u|_{x=0}=\frac{m}{2}\frac{-2d+2d}{d^2+y^2}=0$$The velocity of the fluid along the wall is given by:$$\Huge v|_{x=0}=\frac{2my}{d^2+y^2}$$
This method works for more than just a straight wall. If  the motion of the fluid in a plane is due to a distribution of singularities and there exists a curve $C$ in the plane with no flow across it, then the system of singularities on one side of $C$ is the image of the system on the other side.

Note that this does not work for arbitrary angles of wedges $\alpha$. This is because we need to be able to reflect the real source enough times to fill the plane without adding reflected singularities inside the original fluid domain. Mathematically, this means that $\alpha=\pi/n$ for some $n\in\mathbb{Z}$.

We can also use this method for line vortexes. Take for example a line vortex of strength $k$ at $(0,a)$ above a wall at $y=0$:![[Kinematics of Fluids 2025-10-30 02.43.47.excalidraw]]The velocity potential becomes:$$\Huge\phi=k\tan^{-1}\left(\frac{y-a}{x}\right)-k\tan^{-1}\left(\frac{y+a}{x}\right)$$And the horizontal velocity:$$\Huge\begin{align*} 
u&=\frac{\partial \phi}{\partial x}=k\frac{1}{(\frac{y-a}{x})^2+1}\frac{-(y-a)}{x^2}-k\frac{1}{(\frac{y+a}{x})^2+1}\frac{-(y+a)}{x^2}\\
&=\frac{k(a-y)}{(y-a)^2+x^2}+\frac{k(a+y)}{(y+a^2)+x^2}
\end{align*}$$The first term is from the physical line vortex, and the second is from the image. At the vortex's center, the first term obviously does not contribute, however the second at $(0,a)$ gives:$$\Huge u|_{y=a}=\frac{2ka}{(2a)^2}=\frac{k}{2a}$$Hence the vortex moves itself to the right when placed next to a wall