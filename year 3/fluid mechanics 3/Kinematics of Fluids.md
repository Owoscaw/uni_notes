...
The other option for $u_r=0$ is when $\cos\theta=0$ at $\theta=\pm\pi/2$. Under this condition setting $u_\theta=0$ gives the condition:$$\Huge r^2\mp\frac{\Gamma}{2\pi U}r+a^2=0$$Since $\Gamma<0$ we have all positive coefficients and so there are no positive real solutions for $r$, so we discard this case and look at $\theta=-\pi/2$. This gives the positive branch of this equation, leading to solutions:$$\Huge r=a\left(\frac{|\Gamma|}{4\pi Ua}\pm\sqrt{\left(\frac{\Gamma}{4\pi Ua}\right)^2-1}\right)$$Given $|\Gamma|>4\pi Ua$ we get two real positive roots, one greater than $a$ and one less than $a$. Obviously we discard the solution with $r<a$ as this represents virtual flow within the cylinder. Taking the $r>a$ solution causes the flow to look like:![[Kinematics of Fluids 2025-10-30 02.25.42.excalidraw]]Where we see that the stagnation point is now below the cylinder. This phenomenon is known as backspin.
