...
$e^n$ is an unbounded sequence, so there is $n\in\mathbb{N}$ with $\text{exp}(n)>a$. As the exponential function is increasing, $n$ is an upper bound for $X$. Note that $e^{-n}=\left(\frac{1}{e}\right)^n\to0$ as $n\o$