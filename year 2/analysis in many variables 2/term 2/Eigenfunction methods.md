...
### R modes:
The equation for $R$ is given by:$$\Huge r^2\frac{d^2R}{dr^2}+r\frac{dR}{dr}+(\lambda_nr^2-l^2)R=0$$where we have written $C_\theta=l^2$. We obtain the values of $\lambda_n$ from the $R$ boundary condition:$$\Huge R_n(L_r)=0\implies J_n(L_r\sqrt{\rho_k})=0$$which can be written in full generality as:$$\Huge R_n(R)=\sum_{k=1}^{\infty}A_{l,n}J_l(r\sqrt{\lambda_{l,n}})$$
The full solution then becomes:$$\Huge y_n(r,\theta)=\sum_{l=0}^\infty\sum_{n=1}^\infty J_l(r\sqrt{\lambda_{l,n}})(A_{k,n}\cos(n\pi)+B_{k,n}\sin(n\theta))$$