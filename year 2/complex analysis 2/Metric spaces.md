...
Any sequentially compact set in any metric space is automatically bounded and closed, we show the converse for $\Re^2$ or $\mathbb{C}$. The general case follows by induction. Assume $K$ is bounded and closed. Let $(z_n)_{n\in \mathbb{N}}$ be a sequence in $K$. Writing $z_n=x_n+iy_n$ with $x_n,y_n\in \Re$ we see that:$$\Huge \max(|x_n|,|y_n|)\leq|z_n|\implies(x_n)_{n\in \mathbb{N}},(y_n)_{n\in \mathbb{N}}\text{ are bounded}$$By Bolzano Weierstrass, $(x_n)_{n\in \mathbb{N}}$ has a subsequence $(x_{n_j})_{j\in \mathbb{N}}$ that converges to some $x\in\Re$. Similarly, $(y_{n_j})_{j\in \mathbb{N}}$ is bounded. By Bolzano Weierstrass, there exists $(y_{n_{j_k}})_{k\in \mathbb{k}}$ that converges to some $y\in K$. Note that the sequence $(x_{n_{j_k}})_{k\in \mathbb{N}}$ still converges to $x$, as it is a subsequence of a convergent series. Then take the subequence $(z_{n_{j_k}})_{k\in \mathbb{N}}$ of $(z_n)_{n\in \mathbb{N}}$ and:$$\Huge z_{n_{j_k}}=x_{n_{j_k}}+iy_{n_{j_k}}\to x+iy=z$$Since $(z_{n_{j_k}})_{k\in \mathbb{N}}\subseteq K$ that converges to $z$ and since $K$ is closed, we have $z\in K$. Therefore we can conclude that $K$ is a sequentially compact set.

# Extreme Value Theorem:

Let $f:X\mapsto Y$ be a continuous map between two metric spaces. Then, if $K\subseteq X$ is sequentially compact, the image $f(K)$ is sequentially compact in $Y$